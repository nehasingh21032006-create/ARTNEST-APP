...
const FEATURES = [
  { icon: Percent, title: "0% Upfront Listing Fees" },
  { icon: ShieldCheck, title: "Curatorial Verification & Provenance" },
  { icon: Truck, title: "White-Glove Shipping Concierge" },
];
    ...
    <div className="h-screen w-full bg-[#FDF1E8] flex items-center justify-center p-3 sm:p-6 overflow-hidden">
      <div className="w-full max-w-5xl max-h-[94vh] grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <motion.div
          ...
          className="relative hidden lg:flex flex-col justify-between p-5 sm:p-6 xl:p-8 h-full max-h-[94vh] rounded-2xl overflow-hidden"
          style={{
            backgroundImage:
        ...
        >
          ...
          <div>
            ...
            <h1 className="font-serif text-white text-[26px] xl:text-[32px] leading-tight mb-2">
              Welcome back, Artist
            </h1>
            <p className="text-white/75 text-[13px] max-w-sm mb-5">
              Sign in to manage your listings, track commissions, and review offers from collectors worldwide.
            </p>

            <div className="space-y-2">
              {FEATURES.map(({ icon: Icon, title }) => (
                <div key={title} className="flex items-center gap-2.5">
                  <div className="h-6 w-6 shrink-0 rounded-lg bg-[#A5522F] flex items-center justify-center">
                    <Icon size={12} className="text-white" />
                  </div>
                  <p className="text-white/85 text-[12px] font-medium">{title}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        >
          <motion.h1 variants={item} className="font-serif text-[24px] sm:text-[26px] leading-tight text-stone-900 mb-1.5">
            Sign In to Your Studio
          </motion.h1>
          <motion.p variants={item} className="text-[13px] text-stone-500 mb-4 max-w-sm">
            Access your curatorial dashboard and manage your portfolio.
          </motion.p>
          ...
          <motion.div variants={item} className="mb-3">
            <div className="flex items-baseline justify-between mb-1">
              ...
              <a href="/forgot-password/artist" className="text-[11px] font-medium text-[#A5522F]">Forgot password?</a>
            </div>
          ...
          <motion.p variants={item} className="text-center text-[12px] text-stone-500 mt-3">
            ...
            <a href="/register/artist" className="font-medium text-[#A5522F]">Apply as Artist</a>
          </motion.p>
          ...
          <motion.div variants={item} className="flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-0 mt-4 pt-4 border-t border-stone-100 text-[12px]">
            <span className="text-stone-500">Are you a collector? Then</span>
            <a href="/login/buyer" className="font-medium text-stone-800 flex items-center gap-1">
